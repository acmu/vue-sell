<template>
  <!-- 父节点绑定better-scroll，当子节点内容高度超过父节点高度时，会滑动 -->
  <div class="food" v-show="showFlag" transition="move" v-el:food>
    <div class="food-content">
      ...
      <div class="content">
        ...
        <!-- 给加入购物车一个transition，就不会马上display: none，BS就可以得到 加入购物车 的位置 -->
...
export default {
  ...
  // 父组件可以调用子组件方法，但是子组件不能调用父组件方法
  methods: {
    // 不带下划线的方法命名可能被外部调用，带下划线的方法只会自己用（私有方法）
    show() {
      this.showFlag = true;
      this.selectType = ALL;
...
.food
  ...
  .image-header
    ...
    width: 100%
    height: 0
    // padding设为100%，代表和容器宽度相同，所以使得宽高相等
    padding-top: 100%
    img
      position: absolute
      top: 0
